...
pub fn process_movies(movies_dir: &str, config: &Config) {
    ...
    for (folder, file) in movies {
        progress_bar.println(format!("🎞️ Processing: {}", file));
        ...
        match extractor_result {
            ...
            Err(e) => {
                progress_bar.println(format!("⚠️ Failed to extract clip from {}: {}", file, e))
            }
        }
        progress_bar.inc(1);
    }
...
pub fn process_shows(shows_dir: &str, config: &Config) {
    ...
    if !shows_path.exists() {
        eprintln!("⚠️ Shows directory does not exist: {}", shows_dir);
        return;
    }
    ...
    for show_path in shows {
        ...
        if let Some(latest_season) = seasons.first() {
            ...
            {
                progress_bar.println(format!("   🎞️ Selected episode 1: {}", episode1.display()));

                let backdrop_dir = show_path.join("Backdrops");
                if !backdrop_dir.exists() {
                    progress_bar.println(format!(
                        "   📂 Creating Backdrops folder: {}",
                        backdrop_dir.display()
                    ));
                    fs::create_dir_all(&backdrop_dir).expect("Failed to create Backdrops folder");
                } else {
                    progress_bar
                        .println(format!("📂 Exists Backdrops folder at {:?}", backdrop_dir));
                    progress_bar.println("Skipping..");
                    continue;
                }
                ...
                progress_bar
                    .println(format!("   🎬 Extracting theme to: {}", theme_file.display()));
